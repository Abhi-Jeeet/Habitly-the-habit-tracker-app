...
export default function StreaksScreen() {
  ...
  const [completionHabits, setCompletionHabits] = useState<HabitCompletion[]>(
    []
  );
  const { user } = useAuth();
  ...
  const fetchCompletions = async () => {
    try {
      ...
      setCompletionHabits(completion);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getStreakData = (habitId: string): StreakData => {
    const habitCompletions = completionHabits
